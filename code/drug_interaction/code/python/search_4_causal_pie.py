...
#_F       : indicates the data structure is a flag
...
# Initialization
# @param        x_data_file           source data file, which includes variables that can be the causes, the data are of the following form
#                                  var1 (i.e. header), var1_t1 (i.e. val), ..., var1_tn (i.e. val)
#                                  , ...,
#                                  varn (i.e. header), varn_t1 (i.e. val), ..., varn_tn (i.e. val)
# @param        y_data_file           target data file, which includes variables that can be the effects, the data are of the following form
#                                  var1 (i.e. header), var1_t1 (i.e. val), ..., var1_tn (i.e. val)
#                                  , ...,
#                                  varn (i.e. header), varn_t1 (i.e. val), ..., varn_tn (i.e. val)
def initialization(x_data_file, y_data_file):
    # Load source file
    load_data(x_data_file, True)

    # Load target file
    load_data(y_data_file, False)
...
def get_x_LL():
    for src in x_Dic:
        for win_L in win_LL:
            component_L = [src, win_L[0], win_L[1]]
    ...
    for y in y_Dic:
        ...
        while True:
            ...
            if sample_size_cutoff_met_F is False and sufficient_F is True:
                ...
                # Helper for interaction: update interaction_Dic, discovered_Dic, and the statistics, and output the interaction
                helper_for_interaction(y, X_L, y_cond_X_time_LL)

                # Empty the interaction
                X_L = []
            ...
            # If:
            #     1) enough sample
            # and 2) the combination is not sufficient
            # and 3) no component has been added
            if (sample_size_cutoff_met_F is False
                and sufficient_F is False
                and add_F is False):
                # Expand the combination
                [X_L, y_cond_X_time_LL, expand_F] = expand(y, X_L, y_cond_X_time_LL)

            # If 1) not enough sample or 2) the combination cannot be expanded
            if sample_size_cutoff_met_F is True or expand_F is False:
                # Shrink the combination
                [X_L, y_cond_X_time_LL, shrink_F] = shrink(y, X_L, False)
...
def helper_for_interaction(y, X_L, y_cond_X_time_LL):
    ...
    # Write the combination to spamwriter_X
    spamwriter_X.writerow(['causal combination of ' + y + ': ', X_int_L])
    f_X.flush()

    # Print the combination
    print(['causal combination of ' + y + ': ', X_int_L])
...
if __name__ == "__main__":
    ...
    x_data_file = sys.argv[1]
    y_data_file = sys.argv[2]
    X_file = sys.argv[3]
    log_file = sys.argv[4]
    fig_dir = sys.argv[5]
    ...
    # Initialization
    initialization(x_data_file, y_data_file)
    ...
    with open(log_file, 'w') as f_log:
        ...
        with open(X_file, 'w') as f_X:
            ...
            spamwriter_X = csv.writer(f_X, delimiter=' ')

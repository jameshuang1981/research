...
# @param        src_data_file     discrete data file, which includes variables that can be the causes, the data are of the following form
#                                  var1 (i.e. header), var1_t1 (i.e. val), ..., var1_tn (i.e. val)
#                                  , ...,
#                                  varn (i.e. header), varn_t1 (i.e. val), ..., varn_tn (i.e. val)
# @param        trg_data_file     continuous data file, which includes variables that can be the effects, the data are of the following form
#                                  var1 (i.e. header), var1_t1 (i.e. val), ..., var1_tn (i.e. val)
#                                  , ...,
#                                  varn (i.e. header), varn_t1 (i.e. val), ..., varn_tn (i.e. val)
...
def load_data(src_file, trg_file):
	...
	# e.g. at time 1, "glucose" is "High" and "insulin" is "Low", then src_var_val_L_Dic[1] = ["glucose_High", "insulin_Low"]
	for [var, time_val_LL] in src_var_time_val_LLL:
		for [time, val] in time_val_LL:
			# discrete val is a string representing a category, e.g. "High", "Normal", or "Low"
			# check empty string
			if val:
				if not time in src_var_val_L_Dic:
					src_var_val_L_Dic[time] = []
				# concatenate the val to the name, e.g. var "glucose" with value "High" will be "glucose_High"
				src_var_val_L_Dic[time].append(var + "_" + val)

    # Get time_val_Dic
	# the key is var (i.e. name) and the value is a list of [time, val]
	# e.g. "glucose" is 100 at time 1 and 110 at time 2, then trg_var_time_val_LLL["glucose"] = [[1, 100], [2, 110]]
    for time in src_var_val_L_Dic:
        for src_var_val in src_var_val_L_Dic[time]:
            if not src_var_val in time_val_Dic:
                time_val_Dic[src_var_val] = []
            time_val_Dic[src_var_val].append([time, 1])
	for [var, time_val_LL] in trg_var_time_val_LLL:
		for [time, val] in time_val_LL:
			# target val is a string representing a number, e.g. "100"
			# check string that is not a number
			if is_number(val):
				val = float(val)
				if not var in time_val_Dic:
					time_val_Dic[var] = []
				time_val_Dic[var].append([time, val])

	# get val_Dic
	# the first key is var (i.e. name) and the second key is time
	# the value is val
	# e.g. "glucose" is 100 at time 1, then val_Dic["glucose"][1] = 100
	# this dictionary is used for cases where variables are not measured at every timepoint
	for var in time_val_Dic:
        if not var in val_Dic:
            val_Dic[var] = {}
		for [time, val] in time_val_Dic[var]:
			val_Dic[var][time] = val

	# get alphabet_src_L
	# e.g., ["glucose_High", "insulin_low"]
	for time in src_var_val_L_Dic:
		for src_var_val in src_var_val_L_Dic[time]:
			if not src_var_val in alphabet_src_L:
				alphabet_src_L.append(src_var_val)

	# get alphabet_trg_L
	# e.g., ["glucose', 'insulin"]
	for var in time_val_Dic:
        if not var in alphabet_src_L:
            alphabet_trg_L.append(var)
...
def search():
    for target in alphabet_target_L:
        # Used for BFS
        q = Queue()

        # Initialization, add all slices to queue
        for slices_L in slices_LL:
            q.put(slice_L)
        ...
        while not q.empty():
            ...
            for i in range(size):
                ...
                index = pie_L[i]
                # Visit slices after S_L[index]
                for j in range(index + 1, size):
                    # The slice being considered
                    slice_L = slice_LL[j]

                    # The new pie including the slice
                    pie_inc_L = pie_L.append(slice_L)

                    # Get sample with respect to [pie_L \wedge slice_L] (i.e. the pie including the slice)
                    val_inc_L = get_val_L(e, pie_inc_L, None)
                    # Get sample with respect to [pie_L \wedge \neg slice_L] (i.e. the pie excluding the slice)
                    val_exc_L = get_val_L(e, pie_L, slice_L)

                    # Unpaired t test
                    t, p = stats.ttest_ind(val_inc_L, val_exc_L, equal_var = False)

                    # If the mean of the first sample is significantly larger than that of the second
                    if p < p_threshold:
                        # Add P_Inc_L to queue
                        q.add(P_Inc_L)
...
# get slices
def get_slices():
    for alphabet_src in alphabet_src_L:
        for win_L in win_LL:
            slices_L = [alphabet_scr, win_L[0], win_L[1]]
            slices_LL.append(slices_L)
...
if __name__=="__main__":
	...
	trg_data_file = sys.argv[2]
	pie_file = sys.argv[3]
	p_val_cutoff = float(sys.argv[4])
	...
	load_data(src_data_file, trg_data_file, header, transpose)
...
def get_T_c_L_Dic(c):
  if not c in T_c_L_Dic:
    T_c_L = []
    for time in src_var_val_L_Dic:
      if c in src_var_val_L_Dic[time]:
        T_c_L.append(time)
    T_c_L_Dic[c] = T_c_L
  return T_c_L_Dic[c]
...
def get_E_e_c(e, c_T):
  ...
  if T_e_c_L:
    if not (e, c_T) in E_e_c_Dic:
      val_L = []
      for time in T_e_c_L:
        if time in val_Dic[e]:
          val_L.append(val_Dic[e][time])
